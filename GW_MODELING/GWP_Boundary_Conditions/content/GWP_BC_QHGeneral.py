...
path_quest_ini   = "GW_MODELING/GWP_Boundary_Conditions/questions/initial_general_behavior.json"
path_quest_exer_sc1   = "GW_MODELING/GWP_Boundary_Conditions/questions/exer_general_sc1.json"
path_quest_exer_sc2   = "GW_MODELING/GWP_Boundary_Conditions/questions/exer_general_sc2.json"
path_quest_final = "GW_MODELING/GWP_Boundary_Conditions/questions/final_general_behavior.json"
...
with cent_co:
    st.image('GW_MODELING/GWP_Boundary_Conditions/assets/images/gwp_boundary_Qh_scheme_v2.png')
...
def content_initial():
    ...
    st.info("You can use the initial questions to assess your existing knowledge. (_There is an additional [basic introduction app](https://gwp-boundary-conditions-intro.streamlit.app/) if you want to recap the fundamental knowledge about boundary conditions._)")
...
with st.expander('Show more :blue[**background about the mathematical application of boundary conditions in models**]  **including why RCH and WEL are not sections in this module**', icon ="📑"):
    ...
    **MODFLOW** is the most widely used numerical groundwater modeling code and this module uses MODFLOW's terminology to discuss boundary conditions. The simplest discrete volume in MODFLOW is a six-sided cube (a cell) located in three-dimensional model space by row, column, layer (indices i,j,k) with head solved at the centroid of each cell.
    ...
    left_co, cent_co, last_co = st.columns((20,80,20))
    with cent_co:
        st.image('GW_MODELING/GWP_Boundary_Conditions/assets/images/MF_6_cells.png')
...
with st.expander("Show more :blue[**explanation about the boundary condition types**]", icon ="📑"):
    ...
    **Definition:** The flow across the boundary $Q_B$ is proportional to the difference between the groundwater head $h_{gw}$ at the boundary and the external head of the boundary feature $H_{bc}$ (e.g., a river stage, groundwater head in an adjacent aquifer, or a drain elevation where pressure is atmospheric]). A *conductance factor $Q_B$ controls the flux across the boundary in response to the head gradient* between the model and the boundary. In mathematical terminology, this is a Robin condition.
    ...
    with cc01:
        st.image('GW_MODELING/GWP_Boundary_Conditions/assets/images/GHBintro.png', caption="General concept of a head dependent flow boundary condition (from  McDonald and Harbaugh, 1988)")
...
with st.expander("Show more :blue[**explanation about _Q_-_h_ plots**] that describe boundary condition behavior", icon ="📑", expanded=st.session_state.qh_expander_open):
    ...
    with cc1:
        if modified_plot:
            st.image('GW_MODELING/GWP_Boundary_Conditions/assets/images/Q_h_plots_MF2005_v3.png', caption="Q-h plots describing the behavior of boundary conditions, [adapted from Langevin et al., 2017] (https://doi.org/10.3133/tm6A55.)")
        else:
            st.image('GW_MODELING/GWP_Boundary_Conditions/assets/images/Q_h_plots_MF6_v2.png', caption="Q-h plots describing the behavior of boundary conditions, [modified from Langevin et al., 2017] (https://doi.org/10.3133/tm6A55.)")
...
with lc1:
    st.image('GW_MODELING/GWP_Boundary_Conditions/assets/images/GWF_EX01.jpg')
...
with rc1:
    st.image('GW_MODELING/GWP_Boundary_Conditions/assets/images/GWF_EX02.jpg')
...
with columns_lic[1]:
    st.image('GW_MODELING/GWP_Boundary_Conditions/assets/images/CC_BY-SA_icon.png')